...
def rdf1(df):
    ...
    k=6
    with r2:
        ch1=ui.echart({'xAxis':{'data':gdf['month'].unique().to_list()},'yAxis': {},'series': ll}).style('height:360px;width:780px; margin-top:13px; margin-left:0px; margin-right:6px')
        ch1.options['series'][0]['data']= gdf.filter(pl.col('SALES_DATE')>=datetime(today.year,today.month,1)).filter(pl.col('SALES_DATE')<datetime(today.year,today.month,1)
                                                    +relativedelta(months=12)).sort(['month'],descending=False)['`Fcst DF Final Rev'].to_list()
        ch1.options['series'][0]['name']='DF'
        ch1.options['series'][1]['data']= gdf.filter(pl.col('SALES_DATE')>=datetime(today.year,today.month,1)).filter(pl.col('SALES_DATE')<datetime(today.year,today.month,1)
        ...
        for y in gdf['year'].unique():
            ch1.options['series'][k]['data']= gdf.filter(pl.col('year')==y).group_by('month').sum().sort('month',descending=False)['`Act Orders Rev'].to_list()
        ...
        ch2.options['series'][0]['data']= [[i['SALES_DATE'],i['`Fcst DF Final Rev']] for i in gdf.filter(pl.col('SALES_DATE')>=datetime(today.year,today.month,1)).sort('SALES_DATE',descending=False).iter_rows(named=True)]
        ch2.options['series'][0]['name']='DF'
        ch2.options['series'][1]['data']= [[i['SALES_DATE'],i['`Fcst Stat Final Rev']] for i in gdf.filter(pl.col('SALES_DATE')>=datetime(today.year,today.month,1)).sort('SALES_DATE',descending=False).iter_rows(named=True)]
        ch2.options['series'][1]['name']='Stat'
        ch2.options['series'][2]['data']= [[i['SALES_DATE'],i['`Act Orders Rev']] for i in gdf.sort('SALES_DATE',descending=False).iter_rows(named=True)]
