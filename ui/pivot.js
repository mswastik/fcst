  ...
  callWithJQuery(function($) {
    ...
    aggregatorTemplates = {
      ...
      uniques: function(fn, formatter) {
        ...
        var self = function(attrs) {
          var _self = function(data, rowKey, colKey) {
            return {
              uniq: [],
              push: function(record) {
                var l, len1, attr, ref, x;
                for (l = 0, len1 = attrs.length; l < len1; l++) {
                  attr = attrs[l];
                  x = record[attr];
                  if (ref = x, indexOf.call(this.uniq, ref) < 0) {
                    this.uniq.push(x);
                  }
                }
              },
      ...
      extremes: function(mode, formatter) {
        ...
        var self = function(attrs) {
          var _self = function(data, rowKey, colKey) {
            return {
              val: null,
              sorter: getSort(data != null ? data.sorters : void 0, attrs[0]),
              push: function(record) {
                var ref, ref1, ref2, x;
                x = record[attrs[0]];
      ...
      fractionOf: function(wrapped, type, formatter) {
        ...
        var self = function(attrs) {
          var x;
          x = 1 <= attrs.length ? slice.call(attrs, 0) : [];
          var _self = function(data, rowKey, colKey) {
            return {
              selector: {
                total: [[], []],
        ...
        self.numInputs = wrapped.numInputs;
        return self;
      }
    };
    ...
    PivotData = (function() {
      function PivotData(input, opts) {
        ...
        this.input = input;
        this.valAttrs = (ref = opts.vals) != null ? ref : [];
        this.aggregators = (ref1 = opts.aggregators) != null ? ref1 : [aggregatorTemplates.count()([])];
        ...
        this.rowTotals = {};
        this.colTotals = {};
        this.aggregators = (opts.aggregators || [aggregatorTemplates.count()([])]).map(function(agg) {
          const aggregator = {
            allTotal: agg([]),
            rowTotals: {},
            colTotals: {},
      ...
      PivotData.prototype.processRecord = function(record) {
        var colKey = [], rowKey = [], flatColKey = '', flatRowKey = '', l, len1, len2, n, ref, ref1, ref2, ref3, x;
    ...
    PivotData.prototype.getAggregator = function(rowKey, colKey) {
      var self = this;
      return this.aggregators.map(function(aggregator) {
        var flatRowKey = rowKey.join(String.fromCharCode(0));
        var flatColKey = colKey.join(String.fromCharCode(0));
        var result;
        ftRowKey = rowKey.join(String.fromCharCode(0));
        flatColKey = colKey.join(String.fromCharCode(0));
        
        var getAgg = function(agg) {
          if (rowKey.length === 0 && colKey.length === 0) {
            return agg.allTotal;
          } else if (rowKey.length === 0) {
            return agg.colTotals[flatColKey];
          } else if (colKey.length === 0) {
            return agg.rowTotals[flatRowKey];
          } else {
            return agg.tree[flatRowKey][flatColKey];
          }
        

        return this.aggregators.map(function(agg) {
          var result = getAgg(agg);
      ...
      PivotData.prototype.getAggregator = function(rowKey, colKey) {
        return this.aggregators.map(function(agg) {
          ...
          if (rowKey.length === 0 && colKey.length === 0) {
            ...
            result = agg.colTotals[flatColKey];
          } else if (colKey.length === 0) {
            result = agg.rowTotals[flatRowKey];
          } else {
            result = (agg.tree[flatRowKey] || {})[flatColKey];
          }
    ...
    pivotTableRenderer = function(pivotData, opts) {
      var aggregators, c, colAttrs, colKey, colKeys, defaults, getClickHandler, i, j, r, result, rowAttrs, rowKey, rowKeys, spanSize, tbody, td, th, thead, totalAggregator, tr, txt, val, x, valIdx;
      defaults = {
        table: {
          clickCallback: null,
          rowTotals: true,
      ...
      for (i in rowKeys) {
        ...
        for (j in colKeys) {
          ...
          var aggregators = pivotData.getAggregator(rowKey, colKey) || [];
          aggregators.forEach(function(aggregator, valIdx) {
            val = aggregator.value();
            td = document.createElement("td");
            td.className = "pvtVal row" + i + " col" + j + " val" + valIdx;
      ...
      try {
        ...
        refreshDelayed = (function(_this) {
          return function() {
            ...
            // Get number of inputs needed for aggregator
            var aggregatorFn = opts.aggregators[aggregator.val()];
            var numInputsToProcess = aggregatorFn.numInputs || 1;
            
            vals = [];
            _this.find(".pvtRows li span.pvtAttr").each(function() {
              subopts.rows.push($(this).data("attrName"));
            });
            _this.find(".pvtCols li span.pvtAttr").each(function() {
              subopts.cols.push($(this).data("attrName"));
            });
            ...
            if (inputsNeeded > 0) {
              const pvtVals = _this.find(".pvtVals");
              for (let x = 0; x < inputsNeeded; x++) {
                const newDropdown = $("<select multiple>")
                  .addClass('pvtAttrDropdown')
                  .append($("<option>"))
                  .bind("change", () => refresh());
                
                shownInAggregators.forEach(attr => {
            ...
            if (numInputsToProcess !== 0) {
              ...
              for (x = t = 0, ref5 = numInputsToProcess; 0 <= ref5 ? t < ref5 : t > ref5; x = 0 <= ref5 ? ++t : --t) {
                newDropdown = $("<select multiple>").addClass('pvtAttrDropdown').append($("<option>")).bind("change", function() {
                  return refresh();
                });
            ...
            subopts.vals = vals;
            // Create separate aggregators for each value column with proper initialization
            // Create fresh aggregator instances for each value column
            subopts.aggregators = vals.map(valAttrs =>
              opts.aggregators[aggregator.val()](valAttrs)
            );
